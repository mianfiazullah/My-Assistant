...
export function translateToUrdu(en: string): string {
  ...
  // Extensive dictionary for Punjab police stations (Lahore, Kasur, Sheikhupura, etc.)
  const dict: Record<string, string> = {
    ...
    "faisal town": "فیصل ٹاؤن",
    township: "ٹاؤن شپ",
    "green town": "گرین ٹاؤن",
    "nawan kot": "نواں کوٹ",
    samanabad: "سمن آباد",
    samnabad: "سمن آباد",
    ichra: "اچھرہ",
    ...
    qashqir: "قشقر",
    "qila gujjar singh": "قلعہ گجر سنگھ",
    "gari shahu": "گڑھی شاہو",
    "garhi shahu": "گڑھی شاہو",
    "old anarkali": "اولڈ انارکلی",
    anarkali: "انارکلی",
    naulakha: "نولکھا",
    "bhati gate": "بھاٹی گیٹ",
    "yakki gate": "یکی گیٹ",
    "lohari gate": "لوہاری گیٹ",
    "masti gate": "مستی گیٹ",
    shahdara: "شاہدرہ",
    "shahdara town": "شاہدرہ ٹاؤن",
    "ravi road": "راوی روڈ",
    "badami bagh": "بادامی باغ",
    shadbagh: "شاد باغ",
    baghbanpura: "باغبانپورہ",
    mughalpura: "مغلپورہ",
    ...
    "b type": "بی ٹائپ",
    "c type": "سی ٹائپ",
    "sarwar road": "سرور روڈ",
    "south cantt": "ساؤتھ کینٹ",
    "north cantt": "نارتھ کینٹ",
    "mustafa town": "مصطفی ٹاؤن",
    sabzazar: "سبزہ زار",
    "wahdat colony": "وحدت کالونی",
    "millat park": "ملت پارک",
    shafiqabad: "شفیق آباد",
    sanda: "ساندہ",
    "gulshan-e-ravi": "گلشن راوی",
    "gulshan e ravi": "گلشن راوی",
    sherakot: "شیرا کوٹ",
    "kot lakhpat": "کوٹ لکھپت",
    naseerabad: "نصیر آباد",
    "garden town": "گارڈن ٹاؤن",
    kahna: "کاہنہ",
    "nishtar colony": "نشتر کالونی",
    "quaid-e-azam industrial estate": "قائد اعظم انڈسٹریل اسٹیٹ",
    chuhng: "چوہنگ",
    chung: "چوہنگ",
    "nawab town": "نواب ٹاؤن",
    hanjriwal: "ہنجروال",
    "gawal mandi": "گوالمنڈی",
    gawalmandi: "گوالمنڈی",
    "misri shah": "مصری شاہ",
    ...
    "khayaban-e-amin": "خیابان امین",
    sundar: "سندر",
    liaqatabad: "لیاقت آباد",
    shaqifabad: "شفیق آباد",
    islimpura: "اسلام پورہ",
    islampura: "اسلام پورہ",
    "race course": "ریس کورس",
    "lower mall": "لوئر مال",
    "akbari gate": "اکبری گیٹ",
    "tibbi city": "ٹبی سٹی",
    "mcloed road": "میکلوڈ روڈ",
    shalamar: "شالیمار",
    "dera gujran": "ڈیرہ گجراں",
    gharki: "گھرکی",
    manawan: "مناواں",
    "batapur / manawan": "باٹا پور / مناواں",
    jallo: "جلو",
    lakhodair: "لکھوڈیر",
    manga: "مانگا",
    bhaipheru: "بھائی پھیرو",
    kasoori: "قصوری",
    muridke: "مریدکے",
  ...
  // Try finding words in dictionary (simple replacement)
  let translated = low;

  // Multi-word exact matches that might be partial
  for (const [eng, urd] of Object.entries(dict)) {
    if (eng.length > 4 && translated.includes(eng)) {
      translated = translated.replace(new RegExp(`\\b${eng}\\b`, "g"), urd);
    }
  }
  ...
  for (const [eng, urd] of Object.entries(wordMap)) {
    translated = translated.replace(new RegExp(`\\b${eng}\\b`, "g"), urd);
  }

  // If there are still English letters, we might not have fully translated,
  // but returning the mixed string is better than returning blank,
  // as it saves the user typing the common parts like "Police Station" or "City".
  if (/[a-z]/i.test(translated)) {
    // If we didn't translate anything at all, return empty
    if (translated === low) return "";
    return translated;
  }
...
export default function Admin() {
  ...
  const [webhookUrl, setWebhookUrl] = useState(
    localStorage.getItem("google_sheets_webhook") ||
      "https://script.google.com/macros/s/AKfycbzFThMoqFExs2O_Gry9SrcZ_4W-RuFI7jADKEDf0Rq8LKBgxnO-IpK9yzdsRu-CNerp/exec",
  );
  const [webhookUrl2, setWebhookUrl2] = useState(
    localStorage.getItem("google_sheets_webhook_2") || "",
  );
  ...
  const [sheetTabName, setSheetTabName] = useState(
    localStorage.getItem("google_sheet_sync_tab") || "Form Responses 1",
  );
  ...
  const [expandedScriptUid, setExpandedScriptUid] = useState<string | null>(
    null,
  );
...
function doPost(e) {
  ...
  const headers = [
    ...
    "AC Period Months",
    "Units of AC Period",
    "Detection Period From",
    "Detection Period To",
    "Detection Period Months",
    "Units Assessed",
    ...
    "Seizure Cable Size",
    "Seizure Cable Color",
    "Seizure Cable Length",
    "Checked By",
    "Witnesses",
    "Present Reading at Site",
    "E-Mail Address",
    "Mobile Number",
    "Load Factor",
  ...
  const handleCopyHeaders = () => {
    ...
    navigator.clipboard
      ...
      .then(() => {
        setCopied(true);
        toast.success(
          "Headers copied to clipboard! Paste in Cell A1 of your Sheet.",
        );
  ...
  const syncApprovedUsersSilently = async (
    targetUrl: string,
    targetTab: string,
  ) => {
    ...
    try {
      ...
      const cols = data.table.cols.map((col: any) =>
      ...
      const emailMatch = findBestIndex(
        [
          "e-mail address",
          "email",
          "mail",
      ...
      const subDivMatch = findBestIndex(
        [
          "sub divisional code",
          "sub_divisional_code",
          "sub division",
      ...
      const statusMatch = findBestIndex([
        "status",
        "allow",
        "approved",
        "منظور",
      ...
      const sdoNameMatch = findBestIndex(
        [
          "sdo name",
          "sdo_name",
          "officer name",
      ...
      const sdoNameUrduMatch = findBestIndex(
        [
          "sdo name (urdu)",
          "sdo name(urdu)",
          "sdo name urdu",
      ...
      const designationMatch = findBestIndex(
        ["designation", "post", "scale"],
        ["user", "member", "employee"],
      );
      const sdoCnicMatch = findBestIndex(
        ["sdo cnic no", "sdo cnic", "sdo_cnic", "cnic", "شناختی کارڈ"],
        ["user", "member", "employee"],
      );
      const sdoMobileMatch = findBestIndex(
        [
          "sdo mobile no",
          "sdo mobile",
          "sdo_mobile",
      ...
      const userMobileMatch = findBestIndex(
        ["user mobile", "user phone", "user contact", "user_mobile"],
        ["sdo", "officer"],
      );
      const policeStationMatch = findBestIndex([
        "police",
        "station",
        "thana",
        "تھانہ",
      ...
      const policeStationUrduMatch = findBestIndex([
        "police station (urdu)",
        "police station(urdu)",
        "police station urdu",
        "police_station_urdu",
        "police_stations_urdu",
        "police stations (urdu)",
        "thana urdu",
        "تھانہ اردو",
      ]);
      ...
      if (
        bestEmailColIdx !== -1 &&
        (emailIdx === -1 ||
          colEmailScores[emailIdx] === undefined ||
          colEmailScores[bestEmailColIdx] > (colEmailScores[emailIdx] || 0))
      ) {
      ...
      for (const row of approvedRows) {
        ...
        if (querySnap.empty) {
          ...
          const updateFields = {
            ...
            policeStationsUrdu: row.policeStationsUrdu || [],
            subDivision:
              row.subDivision || existingDoc.data().subDivision || "Gulberg",
          };
          await setDoc(doc(db, "users", existingDoc.id), updateFields, {
            merge: true,
          });
          updatedCount++;
        }
      ...
      if (addedCount > 0 || updatedCount > 0) {
        toast.success(
          `Google Sheet Auto-Sync: Synchronized ${addedCount} new active agents & updated SDO details for ${updatedCount} profiles in the background!`,
          { duration: 5000 },
        );
      }
    } catch (e) {
      console.warn(
        "Silently failed to auto-sync Google Sheet in background:",
        e,
      );
    }
  };
  ...
  const handleSyncSingleUser = async (targetUid: string, userEmail: string) => {
    if (!sheetUrl || !sheetUrl.trim()) {
      toast.error(
        "Please configure the Active Employee Google Sheet under the Google Sheets Synchronizer section first!",
      );
      return;
    }
    ...
    const toastId = toast.loading(
      `Searching & synchronizing latest details for ${userEmail}...`,
    );
    try {
      ...
      if (!rows || rows.length === 0) {
        toast.error(
          "Google Sheet contains no data. Please verify your columns & rows.",
          { id: toastId },
        );
      ...
      if (!matchedRowObj) {
        toast.error(
          `Email "${userEmail}" was not found in active rows of Google Sheet.`,
      ...
      toast.success(
        `Successfully synced & updated settings for ${userEmail}!`,
        { id: toastId, duration: 4000 },
      );
    } catch (e: any) {
      console.error(e);
      toast.error(
        `Search error: ${e.message || "Failed to sync with Google Sheets"}`,
        { id: toastId },
      );
    } finally {
      setSyncingUids((prev) => prev.filter((id) => id !== targetUid));
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.sheetUrl) {
            ...
            syncApprovedUsersSilently(
              data.sheetUrl,
              data.sheetTabName || "Form Responses 1",
            );
          }
        ...
        console.warn(
          "Could not load Google Sheet sync configurations from Firebase, falling back to local cookies:",
          err,
        );
      }
    };
  ...
  const handleSaveSyncSettings = async () => {
    ...
    try {
      ...
      toast.error(`Saved locally, but failed to sync online: ${err.message}`, {
        id: toastId,
      });
    }
  };
  ...
  const fetchSheetData = async (sheetUrl: string, tabName: string) => {
    ...
    if (!sheetId) {
      throw new Error(
        "Invalid Google Sheet URL. Please make sure it is a valid spreadsheet link.",
      );
    }
    ...
    if (!response.ok) {
      throw new Error(
        "Could not fetch sheet data. Please check if the Google Sheet is shared with 'Anyone with the link can view' permission.",
      );
    }
    ...
    if (jsonStart === -1 || jsonEnd === -1) {
      throw new Error(
        "Invalid response format received from Google Sheet visualization layer.",
      );
    }
  ...
  const handleFetchSheetUsers = async () => {
    ...
    try {
      ...
      if (emailMatch.index !== -1) {
        emailMatchSource = emailMatch.isFirstRowHeader
          ? `Header Match ("${firstRowHeaders[emailMatch.index]}")`
          : `Schema Match ("${cols[emailMatch.index]}")`;
      ...
      setSheetDiagnostics({
        ...
        firstRowHeaderLabels:
          firstRowHeaders.length > 0
            ? firstRowHeaders
            : cols.length > 0
              ? cols
              : [],
      ...
      if (mapped.length === 0) {
        toast.warning(
          "No Google Form registrations with valid email addresses found in this sheet.",
          { id: toastId },
        );
      } else {
        toast.success(
          `Connected! Loaded ${mapped.length} users from Google Sheet.`,
          { id: toastId },
        );
      }
      ...
      toast.error(
        `Error: ${err.message}. Please check Sheet URL and make sure anyone with link can view!`,
        { id: toastId },
      );
    } finally {
    ...
    try {
      ...
      if (!querySnap.empty) {
        ...
        const updateFields = {
          ...
          subDivision:
            sheetRow.subDivision || existingDoc.data().subDivision || "Gulberg",
        };
        await setDoc(doc(db, "users", existingDoc.id), updateFields, {
          merge: true,
        });
        toast.success(
          `SDO/Officer details updated instantly for existing account: ${sheetRow.name}!`,
          { id: toastId },
        );
        return;
      }
      ...
      await setDoc(doc(db, "users", placeholderUid), newUserDoc);
      toast.success(
        `${sheetRow.name} successfully registered as active agent!`,
        { id: toastId },
      );
    } catch (err: any) {
  ...
  const handleBulkImportApprovedUsers = async () => {
    ...
    const toastId = toast.loading(
      `Registering ${approvedRows.length} allowed users into active roster...`,
    );
    let addedCount = 0;
    let updatedCount = 0;
    ...
    try {
      ...
      toast.success(
        `Success! Added ${addedCount} new accounts, updated SDO details for ${updatedCount} existing roster entries.`,
        { id: toastId },
      );
    } catch (err: any) {
  ...
  const handleAddNewUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const placeholderUid = `pre-${newUserEmail
        .trim()
        .toLowerCase()
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-white dark:bg-slate-900 rounded-[2rem] border border-neutral-150 dark:border-slate-800 min-h-[60vh] max-w-2xl mx-auto space-y-4 shadow-xl">
        ...
        <p className="text-neutral-500 dark:text-slate-400">
          You are not authorized to access the Admin Panel. This section is
          restricted to administrators and authorized officers of the LESCO
          Detection Engine.
        </p>
        <button
          onClick={() => navigate("/")}
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-neutral-900 dark:text-slate-100">
            Admin Panel
          </h1>
          <p className="text-neutral-500 dark:text-slate-400">
            Manage users, access levels, and system settings
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-neutral-100 dark:border-slate-800 shadow-sm p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-bold text-neutral-900 dark:text-slate-100 mb-2">
                Automatic Google Sheets Connection
              </h2>
              <p className="text-neutral-500 dark:text-slate-400 mb-6 text-sm">
                Paste your Google Apps Script "Web App URL" here to
                automatically save cases to your sheets. You can now use 2
                      ...
                      await fetch("/api/webhook-proxy", {
                        ...
                        body: JSON.stringify({
                          ...
                          payload: getTestPayload({
                            customSubDivision: user?.subDivision,
                          }),
                        }),
                      });
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        toast.success(
                          "Test data sent to Sheet 2 successfully!",
                        );
                      } catch (e) {
              ...
              <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-100 dark:border-amber-900/30 italic text-amber-800 dark:text-amber-400 text-xs">
                <strong>Student Tip:</strong> Remember to set "Who has access"
                to "Anyone" when deploying your script in Google Sheets!
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-neutral-100 dark:border-slate-800 shadow-sm p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-neutral-500 dark:text-slate-400 mb-6 text-sm">
                Need to set up your Google Sheet? Tap the button below to copy
                the header row. When you paste it into Google Sheets (Cell A1),
                it will automatically create separate columns for all fields.
              </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-neutral-500 dark:text-slate-400 mb-4">
                Connect your registration Google Form and Sheet! View
                submissions, check allowed statuses, and register employees as
                active agents automatically.
              </p>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-150/50 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 space-y-2 mb-6 leading-relaxed">
                <span className="font-bold text-emerald-600 dark:text-emerald-400 block mb-1">
                  💡 Easy Connect Guide (بہت آسان طریقہ):
                </span>

                <div className="space-y-3 font-sans">
                  <div className="flex items-start gap-2">
                    <span className="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-110 dark:bg-emerald-900/30 text-emerald-600 text-[10px] font-bold shrink-0 mt-0.5">
                      1
                    </span>
                    <p>
                      اپنا **Google Form** بنائیں جس میں یوزر کا "Name", "Email
                      Address" اور "Sub Division" ہو۔ فارم خود بخود **Google
                      Sheet** میں ریکارڈ سیو کرے گا۔
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p>
                      اپنے Google Sheet میں کالمز کے آخر میں ایک نیا کالم بنائیں
                      جس کا نام **Status**، **Allowed** یا **منظور** رکھیں۔ جس
                      یوزر کو الاؤ کرنا ہو، اس کے آگے **Allow** یا **Yes**
                      لکھیں۔
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p>
                      Google Sheet کے اوپر دائیں کونے میں **Share** بٹن پر کلک
                      کریں اور اسے **"Anyone with the link can view"** (کوئی بھی
                      لنک سے دیکھ سکے) پر سیٹ کریں، تاکہ ایپ ڈیٹا پڑھ سکے۔
                    </p>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p>
                      شیٹ کا لنک اور ٹیب کا نام (مثلاً **Form Responses 1**)
                      نیچے ڈال کر **Fetch** کریں۔ آپ یہاں سے ڈائریکٹ ایک کلک پر
                      انہیں رجسٹر کر سکتے ہیں!
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-slate-200/50 dark:border-slate-800 select-none">
                  <p className="font-urdu text-[14px] text-right text-indigo-600 dark:text-indigo-400 leading-normal">
                    تمام نئے ملازمین کے فارمز کو گوگل پلے شیٹ سے سنک کر کے براہِ
                    راست یہاں لاگو کریں۔
                  </p>
                </div>
              </div>
    ...
    Logger.log("Auto sync failed: " + error.toString());
                ...
                <div className="bg-amber-500/10 dark:bg-amber-500/5 border border-amber-500/20 rounded-2xl p-3.5 space-y-1.5">
                  <p className="text-xs font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5 font-sans">
                    <span>
                      ⚠️ Important Step to Fix Authorization Permission Issue:
                    </span>
                  </p>
                  <p className="text-[11px] text-slate-600 dark:text-slate-350 font-medium leading-relaxed font-sans">
                    Google restricts script triggers from making external
                    requests (like URLs) until authorized. After pasting the
                    Apps Script code, select the <strong>"testWebhook"</strong>{" "}
                    function in the Apps Script toolbar dropdown and click the{" "}
                    <strong>Run</strong> button. Accept permissions via the
                    popup (<em>Go to project &gt; Allow</em>).{" "}
                    <strong>
                      Without this, your form entries will fail silently with
                      permission errors!
                    </strong>
                  </p>
                </div>
                ...
                <p className="text-[11px] text-neutral-600 dark:text-slate-400 font-medium leading-relaxed">
                  Now you don't even need to sync manually! When a new user
                  signs up on your <strong>Google Registration Form</strong> and
                  is approved, their account is{" "}
                  <strong>created instantly in real-time</strong> in the app
                  database.
                </p>
                ...
                <div className="pt-2">
                  ...
                  >
                    {showRegScriptCode
                      ? "Hide script view"
                      : "View registration script code"}
                    <ChevronDown
                      className={cn(
                        "w-3 h-3 transition-transform",
                        showRegScriptCode ? "rotate-180" : "",
                      )}
                    />
                  </button>
  ...
  try {
    ...
    Logger.log("Auto sync failed: " + e.toString());
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-4 border-t border-slate-50 dark:border-slate-850">
                ...
                >
                  <RefreshCw
                    className={cn("w-4 h-4", isLoadingSheet && "animate-spin")}
                  />
                  {isLoadingSheet
                    ? "CONNECTING & SCANNING SPREADSHEET..."
                    : "FETCH FORM REGISTRATIONS"}
                </button>
                ...
                {sheetRows.length > 0 && (
                  ...
                  >
                    ...
                    REGISTER {sheetRows.filter((r) => r.isAllowed).length}{" "}
                    ALLOWED USERS (BULK SYNC)
                  </button>
                )}
              </div>
                ...
                <div className="mt-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3">
                  <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                    ...
                    <h4 className="text-xs font-bold uppercase tracking-wider">
                      Sheet Scanning Diagnostics Log
                    </h4>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px] font-mono text-slate-600 dark:text-slate-400">
                    <div>
                      ...
                      <span className="font-bold text-slate-850 dark:text-slate-200">
                        {sheetDiagnostics.totalRowsFetched}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">
                        Is Header Row Ignored?
                      </span>
                      <span className="font-bold text-slate-850 dark:text-slate-200">
                        {sheetDiagnostics.isFirstRowActuallyHeader
                          ? "Yes (Row #1)"
                          : "No"}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-bold text-slate-850 dark:text-slate-200">
                        Col #{sheetDiagnostics.emailIdx + 1}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">
                      ...
                      <span className="font-bold text-slate-850 dark:text-slate-200 break-all text-xs text-indigo-500">
                        {sheetDiagnostics.emailMatchSource}
                      </span>
                    </div>
                  </div>
                  ...
                  {sheetDiagnostics.firstRowHeaderLabels.length > 0 && (
                    <div className="text-[11px] font-sans text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-950 p-2 text-xs rounded border border-slate-100 dark:border-slate-850 max-h-32 overflow-y-auto">
                      <span className="font-semibold block mb-1 text-slate-400 text-[10px] uppercase font-mono tracking-wider">
                        Detected Columns:
                      </span>
                      <div className="flex flex-wrap gap-1.5 font-mono">
                        {sheetDiagnostics.firstRowHeaderLabels.map(
                          (lbl, idx) => (
                            <span
                              key={idx}
                              className={cn(
                  ...
                  {sheetDiagnostics.validEmailsCount === 0 && (
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded p-3 text-xs text-amber-700 dark:text-amber-400 leading-relaxed font-sans">
                      <strong>💡 Troubleshooting Guide:</strong> If no
                      registrations were processed, confirm that:
                      <ul className="list-disc pl-5 mt-1 space-y-1">
                        <li>
                          Your Selected Tab Name (
                          <strong>"{sheetTabName}"</strong>) matches your sheet
                          name/tab perfectly.
                        </li>
                        <li>
                          The Google Form sheet contains at least one completed
                          submission with a valid email.
                        </li>
                        <li>
                          For privacy settings, ensure the sheet has been
                          published/shared as{" "}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-6 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-slate-400">
                  Pre-register, activate, disable, or custom-configure LESCO
                  subdivisions and sheets webhook rules for all employee
                  accounts.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1.5 rounded-full border border-indigo-100 dark:border-indigo-900/10 shrink-0">
                {
                  activeUsersList.filter((u) => !u.uid.startsWith("pre-"))
                    .length
                }{" "}
                Google Accounts
              </span>
            ...
            {showAddUserForm && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1 font-sans">
                      Google Sheets Webhook (2 - Optional)
                    </label>
                    <input
                      type="url"
                      placeholder="https://script.google.com/macros/s/.../exec"
          ...
          {activeUsersList.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-center bg-slate-50 dark:bg-slate-800/10 rounded-2xl p-6 border border-dashed border-slate-200 dark:border-slate-800">
              ...
              <p className="text-sm font-bold text-slate-500 dark:text-slate-400 animate-pulse">
                Loading active employee registry...
              </p>
            </div>
          ) : (
            (() => {
              const filtered = activeUsersList.filter(
                (u) =>
                  u.name
                    .toLowerCase()
                    .includes((usersSearchFilter || "").toLowerCase()) ||
                  u.email
                    .toLowerCase()
                    .includes((usersSearchFilter || "").toLowerCase()) ||
                  (u.subDivision || "")
              ...
              if (filtered.length === 0) {
                return (
                  <div className="flex flex-col items-center justify-center p-12 text-center bg-slate-50/50 dark:bg-slate-800/10 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <h4 className="text-sm font-bold text-neutral-900 dark:text-slate-200 mb-1">
                      No matching users found
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Try adjusting your spelling or division searches.
                    </p>
                  </div>
                );
              }
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          <div
                            className={cn(
                              "w-12 h-12 flex items-center justify-center rounded-2xl font-bold border text-sm shrink-0 shadow-sm select-none",
                              assignedColor,
                            )}
                          >
                            {initials}
                        ...
                        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100 dark:border-slate-800/60 text-xs">
                          <div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5 font-sans select-none">
                              Sub Division
                            </span>
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  setEditDraft((prev) =>
                                    prev
                                      ? { ...prev, subDivision: e.target.value }
                                      : null,
                                  )
                                }
                        ...
                        <div className="pt-3 border-t border-slate-100 dark:border-slate-800/60 text-xs space-y-2">
                          <div>
                            ...
                            {isEditing ? (
                              ...
                              <div className="flex items-center gap-2">
                                <span
                                  className="font-mono text-[10px] text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 px-2 py-1 rounded-md block truncate border border-slate-100 dark:border-slate-800/80 flex-1"
                                  title={u.webhookUrl || "Not Configured"}
                                >
                                  {u.webhookUrl ||
                                    "Not Configured (Using Default Webhook)"}
                                </span>
                                <button
                                  type="button"
                                  onClick={async () => {
                                    ...
                                    if (!targetWebhook) {
                                      toast.error(
                                        "Webhook 1 is required to run a test.",
                                      );
                                      return;
                                    }
                                    try {
                                      toast.info(
                                        `Sending test data to ${u.name}'s Sheet 1...`,
                                      );
                                      await fetch("/api/webhook-proxy", {
                                        method: "POST",
                                        headers: {
                                        ...
                                        body: JSON.stringify({
                                          ...
                                          payload: getTestPayload({
                                            customSubDivision: u.subDivision,
                                          }),
                                        }),
                                      });
                                      toast.success(
                                        `Success! Test data sent to ${u.name}'s Webhook 1`,
                                      );
                                    } catch (e) {
                                      toast.error("Failed to send test data.");
                                    }
                            ...
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  setEditDraft((prev) =>
                                    prev
                                      ? { ...prev, webhookUrl2: e.target.value }
                                      : null,
                                  )
                                }
                                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1 text-[10px] text-slate-900 dark:text-slate-101 font-mono focus:ring-1 focus:ring-purple-500 focus:outline-none"
                              ...
                              <div className="flex items-center gap-2">
                                <span
                                  className="font-mono text-[10px] text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 px-2 py-1 rounded-md block truncate border border-slate-100 dark:border-slate-800/80 flex-1"
                                  title={u.webhookUrl2 || "None (Optional)"}
                                >
                                ...
                                {(u.webhookUrl2 || webhookUrl2) && (
                                  <button
                                    ...
                                    onClick={async () => {
                                      ...
                                      try {
                                        toast.info(
                                          `Sending test data to ${u.name}'s Sheet 2...`,
                                        );
                                        await fetch("/api/webhook-proxy", {
                                          method: "POST",
                                          headers: {
                                        ...
                                        toast.success(
                                          `Success! Test data sent to ${u.name}'s Webhook 2`,
                                        );
                                      } catch (e) {
                                        toast.error(
                                          "Failed to send test data to Sheet 2.",
                                        );
                                      }
                                    }}
                                    className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/40 dark:hover:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30 rounded-lg font-bold text-[10px] cursor-pointer shrink-0 transition-all select-none"
                        ...
                        {/* SDO / Officer Details */}
                        <div className="pt-3 border-t border-slate-100 dark:border-slate-800/60 text-xs space-y-2">
                          <span className="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-wider block font-sans select-none">
                            SDO / Officer Details
                          </span>
                          <div className="grid grid-cols-2 gap-2 bg-slate-50/50 dark:bg-slate-950/20 p-2.5 rounded-xl border border-slate-100 dark:border-slate-850">
                            <div>
                              <span className="text-[9px] font-bold text-slate-400 uppercase block select-none mb-0.5">
                                SDO Name
                              </span>
                              {isEditing ? (
                                <input
                            ...
                            <div>
                              ...
                              {isEditing ? (
                                ...
                                <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400 urdu-font block truncate">
                                  {u.sdoNameUrdu || "درج نہیں"}
                                </span>
                              )}
                            </div>
                            <div>
                              ...
                              {isEditing ? (
                                ...
                                <span className="text-xs font-semibold text-slate-650 dark:text-slate-400 block truncate">
                                  {u.designation || "Not Loaded"}
                                </span>
                              )}
                            </div>
                            <div>
                              ...
                              {isEditing ? (
                                ...
                                <span className="text-xs font-mono text-slate-600 dark:text-slate-450 block truncate">
                                  {u.sdoCnic || "Not Loaded"}
                                </span>
                              )}
                            </div>
                            <div className="col-span-2">
                              ...
                              {isEditing ? (
                                ...
                                <span className="text-xs font-mono text-slate-600 dark:text-slate-450 block truncate">
                                  {u.sdoMobile || "Not Loaded"}
                                </span>
                              )}
                            </div>
                            <div>
                              <span className="text-[9px] font-bold text-slate-400 uppercase block select-none mb-0.5">
                                Police Station English
                              </span>
                              {isEditing ? (
                                <input
                                  type="text"
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setEditDraft((prev) => {
                                      ...
                                      const currentUrduList = [
                                        ...(prev.policeStationsUrdu || []),
                                      ];
                                ...
                                <span
                                  className="text-xs font-semibold text-slate-650 dark:text-slate-400 block truncate"
                                  title={
                                    u.policeStations?.join(", ") ||
                                    u.policeStation
                                  }
                                >
                                  {u.policeStations &&
                                  u.policeStations.length > 1
                                    ? u.policeStations.join(", ")
                                    : u.policeStation || "Not Loaded"}
                                </span>
                              )}
                            </div>
                            <div>
                              <span className="text-[9px] font-bold text-slate-400 uppercase block select-none mb-0.5 urdu-font">
                                نام تھانہ (اردو)
                              </span>
                              {isEditing ? (
                                <input
                                ...
                                <span
                                  className="text-xs font-semibold text-slate-800 dark:text-slate-200 block truncate urdu-font"
                                  title={
                                    u.policeStationsUrdu?.join("، ") ||
                                    u.policeStationUrdu
                                  }
                                >
                                  {u.policeStationsUrdu &&
                                  u.policeStationsUrdu.length > 1
                                    ? u.policeStationsUrdu.join("، ")
                                    : u.policeStationUrdu || "درج نہیں"}
                                </span>
                              )}
                            </div>
                            ...
                            {isEditing && (
                              <div className="col-span-2 pt-3 border-t border-slate-100 dark:border-slate-800 mt-2 space-y-2">
                                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase block select-none">
                                  Manage Multiple Police Stations / ایک سے زیادہ
                                  تھانے شامل کریں
                                </span>

                                {/* Display List of Police Stations */}
                                <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
                                  {(editDraft?.policeStations || []).length >
                                  0 ? (
                                    (editDraft?.policeStations || []).map(
                                      (ps, idx) => {
                                        const ur =
                                          editDraft?.policeStationsUrdu?.[
                                            idx
                                          ] || "";
                                        return (
                                          <div
                                            key={idx}
                                            className="flex items-center justify-between gap-2 bg-slate-50 dark:bg-slate-950 p-1.5 rounded-lg border border-slate-100 dark:border-slate-800"
                                          >
                                    ...
                                    <span className="text-[10px] text-slate-400 italic block px-1">
                                      None registered (uses the fallback /
                                      primary station above).
                                    </span>
                                  )}
                                </div>
                                  ...
                                  <div>
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const newVal = e.target.value;
                                        const translated =
                                          translateToUrdu(newVal);
                                        const urInput = document.getElementById(
                                          "new-ps-ur",
                                        ) as HTMLInputElement;
                                        if (urInput) {
                                  ...
                                  <div className="flex gap-1.5">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        if (!enVal) {
                                          toast.error(
                                            "Police station name (English) is required!",
                                          );
                                          return;
                                        }
                                        ...
                                        if (currentPS.includes(enVal)) {
                                          toast.error(
                                            "This police station is already added!",
                                          );
                                          return;
                                        }

                                        const autoUrdu =
                                          urVal || translateToUrdu(enVal) || "";
                                        ...
                                        setEditDraft((prev) =>
                                          prev
                                            ? {
                                                ...
                                                policeStation:
                                                  prev.policeStation || enVal,
                                                policeStationUrdu:
                                                  prev.policeStationUrdu ||
                                                  autoUrdu,
                                              }
                                            : null,
                                        );
                                        ...
                                        toast.success(
                                          "Added police station to user settings.",
                                        );
                                      }}
                        ...
                        {!isEditing && (
                          <div className="pt-2 border-t border-slate-150/40 dark:border-slate-800/40">
                            <button
                              type="button"
                              onClick={() =>
                                setExpandedScriptUid(
                                  expandedScriptUid === u.uid ? null : u.uid,
                                )
                              }
                                ...
                                >
                                  <div className="bg-slate-50 dark:bg-slate-950/70 border border-slate-100 dark:border-slate-800 p-3 rounded-2xl select-text">
                                    <p className="font-bold text-slate-800 dark:text-slate-200 mb-1 font-sans">
                                      Pre-configured setup guide for:{" "}
                                      <span className="text-purple-600 dark:text-purple-400">
                                        {u.name} ({u.subDivision || "Default"})
                                      </span>
                                    </p>
                                    <ol className="list-decimal pl-4.5 space-y-1 text-slate-500 dark:text-slate-400 font-medium font-sans">
                                      ...
                                      <li>
                                        Deploy as "Web App" accessible to
                                        "Anyone".
                                      </li>
                                      <li>
                                        Copy Web App URL & enter as user Webhook
                                        1!
                                      </li>
                                    </ol>
                                  </div>
  ...
  try {
    ...
    return ContentService.createTextOutput(JSON.stringify({ "success": false, "error": e.toString() })).setMimeType(ContentService.MimeType.JSON);
                                          ...
                                          navigator.clipboard
                                            ...
                                            .then(() => {
                                              toast.success(
                                                `Copied preconfigured script for ${u.name}!`,
                                              );
                                            });
                                        }}
                          ...
                          {isEditing ? (
                            ...
                            <>
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleSyncSingleUser(u.uid, u.email)
                                }
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleToggleUserDisabled(u.uid, !!u.disabled)
                                }
                                className={cn(
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  impersonateUser(u.uid);
                                  navigate('/new-case');
                                }}
                                className="flex-1 py-1.5 bg-amber-50 dark:bg-amber-950/45 hover:bg-amber-100/80 hover:text-amber-600 border border-amber-100/50 dark:border-amber-900/30 text-amber-550 dark:text-amber-400 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                              >
                              ...
                              {u.role !== "admin" &&
                                u.email?.toLowerCase() !==
                                  ...
                                  <button
                                    ...
                                    className={cn(
                                      "px-3 py-1.5 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 border",
                                      deleteConfirmUid === u.uid
                                        ? "bg-red-500 text-white border-red-600 hover:bg-red-600 focus:bg-red-600 focus:ring focus:ring-red-500/30"
                                        : "bg-red-50 dark:bg-red-950/45 hover:bg-red-100/80 hover:text-red-700 border-red-100/50 dark:border-red-900/30 text-red-500 dark:text-red-400",
                                    )}
                                  >
                                    {deleteConfirmUid === u.uid ? (
                                      <>
