...
export default function NewCase() {
  ...
  const handleBulkUploadToDrive = async () => {
    ...
    try {
      ...
      let subDivCodeForFolder = "";
      const rawRefForFolder = detectionData?.referenceNumber || billData?.referenceNumber || referenceNumber || "";
      const matchForFolder = String(rawRefForFolder).match(/\b\d{5}\b/);
      if (matchForFolder) {
        subDivCodeForFolder = matchForFolder[0];
      }
      const finalSubDivision = subDivCodeForFolder || billData?.subDivisionName || user?.subDivision || "";
  ...
  const saveCase = async () => {
    ...
    try {
      ...
      let subDivCodeForSave = "";
      const rawRefSave = newCase.referenceNumber || billData?.referenceNumber || referenceNumber || "";
      const matchSave = String(rawRefSave).match(/\b\d{5}\b/);
      if (matchSave) {
        subDivCodeForSave = matchSave[0];
      }
      const finalSubDivision = subDivCodeForSave || billData?.subDivisionName || user?.subDivision || "";
